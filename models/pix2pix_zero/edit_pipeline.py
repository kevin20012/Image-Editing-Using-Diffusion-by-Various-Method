...
class EditingPipeline(BasePipeline):
    def __call__(
        ...
        negative_prompt_embeds: Optional[torch.FloatTensor] = None, #for null-text-inversion
            ...
            with self.progress_bar(total=num_inference_steps) as progress_bar:
                for i, t in enumerate(timesteps):
                    ...
                    if negative_prompt_embeds is not None:
                        prompt_embeds_ = torch.cat([negative_prompt_embeds[i].expand(1, *prompt_embeds[1].shape), prompt_embeds[1].unsqueeze(0)])
                        # predict the noise residual
                        noise_pred = self.unet(latent_model_input,t,encoder_hidden_states=prompt_embeds_,cross_attention_kwargs=cross_attention_kwargs,).sample
                    else:
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                # predict the noise residual
                if negative_prompt_embeds is not None:
                    prompt_embeds_ = torch.cat([negative_prompt_embeds[i].expand(1, *prompt_embeds_edit[1].shape), prompt_embeds_edit[1].unsqueeze(0)])
                    noise_pred = self.unet(x_in,t,encoder_hidden_states=prompt_embeds_.detach(),cross_attention_kwargs=cross_attention_kwargs,).sample
                else:
                ...
                for name, module in self.unet.named_modules():
                    ...
                    if module_name == "Attention" and 'attn2' in name:
                        ...
                        loss = loss + ((curr-ref)**2).sum((1,2)).mean(0)
                # opt.zero_grad()
                ...
                with torch.no_grad():
                    if negative_prompt_embeds is not None:
                        noise_pred = self.unet(x_in.detach(),t,encoder_hidden_states=prompt_embeds_,cross_attention_kwargs=cross_attention_kwargs,).sample
                    else:
